...
const SHELL_RADIUS = 2.65

const SHELL_JITTER = 0
const SHELL_TILT = 0
const SHELL_SIZE_VAR = 0

const SHELL_DESKTOP = { rows: 8, scale: 3.2 }
const SHELL_MOBILE = { rows: 7, scale: 3.5 }
...
export function JellyCanvas({
  ...
  const refs = useRef({
    ...
    heroText: null as THREE.Mesh | null,
    hoveredRing: null as number | null,
  ...
  useEffect(() => {
    ...
    gltfLoader.load('/models/segRoman.glb', (seg) => {
      ...
      const sharedMat = makeGlass({
        ...
        iridescenceIOR: 1.35,
        envMapIntensity: 3.8,
        attenuationColor: new THREE.Color('#94bfe3'),
      ...
      for (let row = 0; row < SHELL_ROWS; row++) {
        ...
        const perBand = Math.max(4, Math.ceil((Math.PI * 2) / angW))
    ...
    const RING_DATA = [
      {
        bullets: ['• AI & AUDIOVISUAL', '• 3D MICROSITE', '• BRAND ACTIVATION'],
        desc: ['AN IMMERSIVE AI AUDIOVISUAL', 'EXPERIENCE WHERE EVERY NOTE AND BEAT', 'SHAPES YOUR VIRTUAL SURROUNDINGS.'],
      },
      {
        bullets: ['• REALTIME GLASS', '• PRODUCT CONFIGURATOR', '• BESPOKE SHADERS'],
        desc: ['NEXT-GENERATION REALTIME PRODUCT', 'GLASS POWERED BY VOLUMETRIC LIGHTING', 'AND PHYSICAL SIMULATION.'],
      },
      {
        bullets: ['• COMPUTER VISION', '• LUXURY ATELIER', '• INTERACTIVE KIOSK'],
        desc: ['HYPER-SENSORY SPATIAL COMPUTING', 'FOR VISIONARY BRANDS MERGING HARDWARE', 'AND UNRIVALED CREATIVE CODE.'],
      },
      {
        bullets: ['• WEBAR & XR', '• MIXED REALITY', '• GENERATIVE MOTION'],
        desc: ['BROWSER-BASED AUGMENTED REALITY', 'PUSHING THE BOUNDARIES OF PERFORMANCE', 'AND SENSORY IMMERSION.'],
      },
    ]

    const makeBandTexture = (idx: number) => {
      ...
      cx.fillStyle = 'rgba(240, 245, 255, 0.94)'
      cx.fillRect(0, 0, c.width, c.height)

      cx.fillStyle = 'rgba(5, 8, 15, 0.15)'
      cx.fillRect(0, 0, c.width, 3)
      cx.fillRect(0, c.height - 3, c.width, 3)

      const reps = 3
      const slotW = c.width / reps
      const data = RING_DATA[idx % RING_DATA.length]

      for (let i = 0; i < reps; i++) {
        const x0 = i * slotW

        cx.fillStyle = '#05080f'
        cx.font = '900 178px "Impact", "DrukMedium", "Syne", sans-serif'
        cx.textAlign = 'left'
        cx.textBaseline = 'top'
        cx.fillText('COMING SOON', x0 + 40, 36)

        cx.fillStyle = 'rgba(5, 8, 15, 0.18)'
        cx.fillRect(x0 + 40, 290, slotW - 80, 2)
        ...
        for (let r = 0; r < 3; r++) {
          for (let col = 0; col < 4; col++) {
            if ((r + col) % 2 === 0) {
              cx.fillRect(iconX + col * 9, iconY + r * 14, 6, 6)
            }
          }
        }

        cx.font = '700 24px "IBM Plex Mono", monospace'
        cx.textAlign = 'left'
        cx.textBaseline = 'top'
        cx.fillStyle = '#05080f'
        data.bullets.forEach((b, bi) => {
          cx.fillText(b, x0 + 95, 318 + bi * 32)
        })

        cx.font = '600 20px "IBM Plex Mono", monospace'
        cx.fillStyle = 'rgba(5, 8, 15, 0.82)'
        const descX = x0 + 560
        data.desc.forEach((d, di) => {
          cx.fillText(d, descX, 320 + di * 30)
        })

        cx.fillStyle = 'rgba(5, 8, 15, 0.25)'
        cx.fillRect(x0 + slotW - 2, 40, 2, c.height - 80)
      }
    ...
    const RING_R = 1.95
    const torusGeo = new THREE.TorusGeometry(RING_R, 0.11, 24, 128)
    const bandGeo = new THREE.CylinderGeometry(RING_R + 0.16, RING_R + 0.16, 1.05, 128, 1, true)
    const glowGeo = new THREE.TorusGeometry(RING_R, 0.26, 20, 96)
    ...
    PROJECTS.forEach((proj, idx) => {
      ...
      const torusTop = new THREE.Mesh(torusGeo, ringGlass)
      torusTop.position.z = 0.525
      g.add(torusTop)

      const torusBot = new THREE.Mesh(torusGeo, ringGlass)
      torusBot.position.z = -0.525
      g.add(torusBot)
    ...
    const buildTypography = (font: Font) => {
      ...
      const heroTextGeo = new TextGeometry('AURELIA LABS', {
        font,
      ...
      heroTextGeo.computeBoundingBox()
      heroTextGeo.center()

      const heroTextMat = new THREE.MeshBasicMaterial({
        color: 0x0a0f1a,
        toneMapped: false,
        transparent: true,
        opacity: 0,
      })
      const heroText = new THREE.Mesh(heroTextGeo, heroTextMat)
      heroText.position.set(0, -0.05, -5)
      heroText.visible = false
      scene.add(heroText)
      refs.current.heroText = heroText
    ...
    const onMouseMove = (e: MouseEvent) => {
      ...
      if (!r.isEntered) {
        ...
        raycaster.setFromCamera(_ndc.set(r.mx, r.my), camera)
        const bands = r.rings.map((rg) => rg.band)
        const hits = raycaster.intersectObjects(bands, false)
        if (hits.length > 0) {
          const hitObj = hits[0].object
          const found = r.rings.find((rg) => rg.band === hitObj)
          const hitIdx = found !== undefined ? found.index : null
          if (hitIdx !== r.hoveredRing) {
            r.hoveredRing = hitIdx
            onHoverRing?.(hitIdx)
          }
        } else if (r.hoveredRing !== null) {
          r.hoveredRing = null
          onHoverRing?.(null)
        }
      } else if (r.hoveredRing !== null) {
    ...
    const onMouseLeave = () => {
      ...
      if (refs.current.hoveredRing !== null) {
        refs.current.hoveredRing = null
        onHoverRing?.(null)
      }
    }
    ...
    const animate = () => {
      ...
      if (!entered) {
        ...
        if (r.heroText) {
          r.heroText.visible = r.assemble > 0.05
          const heroFade = easeOutCubic(clamp01((r.assemble - 0.1) / 0.6))
          ;(r.heroText.material as THREE.MeshBasicMaterial).opacity = heroFade * 0.9
        }
        ...
        if (s < TL.rotate.end) {
          ...
          const camTrackY = lerp(WORLD.ringsExitY, WORLD.aureliaY, easeOutCubic(clamp01(p / 0.5)))
          _followPos.set(r.plx * 0.5, camTrackY - 0.75, 7.85)
          ...
          _followLook.set(0, camTrackY + 0.35, 0)
          _lockedLook.set(0, WORLD.aureliaY, 1.4)
          tLook.copy(_followLook).lerp(_lockedLook, hand)
          tRoll = 0

          const climb = Math.pow(p, 1.5)
          const jy = WORLD.ringsExitY + climb * 75 + p * 12
          const jz = -p * 24
          const jx = Math.sin(p * Math.PI) * 0.8
          jellyPos.set(jx, jy, jz)
          jellyRot.set(-0.6 - p * 0.25, t * 0.35 + p * Math.PI * 1.6, Math.sin(p * Math.PI * 2) * 0.08)
          jellyScale = lerp(0.58, 0.02, Math.pow(p, 1.3))

          const vanishFade = clamp01(1 - (p - 0.35) / 0.55)
          if (r.jellyMatOuter) {
            r.jellyMatOuter.opacity = configRef.current.opacity * vanishFade
          }
          if (r.jellyMatInner) {
            r.jellyMatInner.opacity = 0.9 * vanishFade
          }
          jellyGroup.visible = p < 0.98
        }

        if (s < TL.finale.start) {
          jellyGroup.visible = true
          if (r.jellyMatOuter) r.jellyMatOuter.opacity = configRef.current.opacity
          if (r.jellyMatInner) r.jellyMatInner.opacity = 0.9
        }
      ...
      r.rings.forEach((ring) => {
        ...
        const near = clamp01(1 - Math.abs(d) / 2.6)
        ring.group.rotation.z = t * ring.spin + ring.index * 0.7
        ring.group.position.y = ring.y + Math.sin(t * 1.1 + ring.index * 1.5) * 0.1

        const bandMat = ring.band.material as THREE.MeshBasicMaterial

        bandMat.opacity = easeOutCubic(near) * 0.65
        if (bandMat.map) bandMat.map.offset.x = (t * 0.035 + ring.index * 0.25) % 1

        const through = clamp01(1 - Math.abs(jellyGroup.position.y - ring.y) / 2.0)
        const glowMat = ring.glow.material as THREE.MeshBasicMaterial
        glowMat.opacity = easeOutCubic(through) * 0.35
      ...
      if (r.heroText && entered) {
        r.heroText.visible = false
      }
