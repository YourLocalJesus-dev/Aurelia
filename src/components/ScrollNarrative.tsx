...
export function ScrollNarrative({ isEntered }: Props) {
  ...
  useEffect(() => {
    ...
    const unsub = scrollStore.subscribe((s, vel) => {
      ...
      const inRings = s >= TL.rings.start - 0.01 && s <= TL.rings.end + 0.01
      if (ringHud) {
        const fade =
          easeOutCubic(clamp01((s - (TL.rings.start - 0.01)) / 0.025)) *
          (1 - easeOutCubic(clamp01((s - (TL.rings.end - 0.025)) / 0.03)))
      ...
      >
        ...
        <div className="ks-rings-inner">
          ...
          <div className="ks-rings-meta">
            <h3 data-ring-title>COMING SOON</h3>
            <p data-ring-cat>EXPERIMENT 01 · IN DEVELOPMENT</p>
          </div>
        ...
        <div className="ks-hud-chapters">
          {['ROTATE', 'ASCEND', 'WORK', 'ARRIVE'].map((c, idx) => (
            <span
              className="ks-hud-chapter"
              data-chapter
              key={c}
              style={{ cursor: 'pointer' }}
              onClick={() => {
                const trackEl = document.querySelector<HTMLElement>('.home-page')
                const max = trackEl
                ...
                const targetP =
                  idx === 0
                    ...
                    : idx === 2
                    ? 0.59
                    : TL.finale.start + 0.05
                window.scrollTo({ top: max * targetP, behavior: 'smooth' })
              }}
            >
              {c}
            </span>
