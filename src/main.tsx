...
function App() {
  ...
  const [toast, setToast] = useState<{ visible: boolean; text: string; sub?: string }>({
    visible: false,
    text: '',
    sub: '',
  })
  ...
  const showToast = (text: string, sub?: string) => {
    ...
    setToast({ visible: true, text, sub })
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      ...
      if (!isEntered && isAssembled) {
        const interactive = target.closest('header, a, button, .menu-switch, .volume, .bottom-jel-badge')
        if (!interactive) {
    ...
    <div className="aurelia-app">
      ...
      {isPreloaderDone && (
        <JellyCanvas
          ...
          onHoverRing={(ringIndex) => {
            if (ringIndex !== null) {
              setCursorHover('VIEW', true)
            } else {
              resetCursor()
            }
          }}
          onRingClick={(ringIndex) => {
            showToast('Coming Soon!', 'Project archive in active development')
          }}
        />
      )}
      ...
      <div className={`coming-soon-toast ${toast.visible ? 'is-active' : ''}`}>
